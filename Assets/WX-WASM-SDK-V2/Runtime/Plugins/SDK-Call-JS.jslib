...
WX_SyncFunction_tnn: function(functionName, returnType, param1, param2){
    var res = window.WXWASMSDK.WX_SyncFunction_tnn(_WXPointer_stringify_adaptor(functionName), _WXPointer_stringify_adaptor(returnType), param1, param2);
    var bufferSize = lengthBytesUTF8(res || '') + 1;
    var buffer = _malloc(bufferSize);
    stringToUTF8((res || ''), buffer, bufferSize);
    return buffer;
...
WX_SyncFunction_tnnt: function(functionName, returnType, param1, param2, param3){
    var res = window.WXWASMSDK.WX_SyncFunction_tnnt(_WXPointer_stringify_adaptor(functionName), _WXPointer_stringify_adaptor(returnType), param1, param2, _WXPointer_stringify_adaptor(param3));
    var bufferSize = lengthBytesUTF8(res || '') + 1;
    var buffer = _malloc(bufferSize);
    stringToUTF8((res || ''), buffer, bufferSize);
    return buffer;
